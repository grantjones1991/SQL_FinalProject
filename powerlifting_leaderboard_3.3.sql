...
-- (this is a sum of all lifts done by an athlete, so it compares total of someone who's done let's say 1 lift to someone
-- who's done 20 lifts)

SELECT a.athlete_name, SUM(ll.weight_pounds) AS total_weight
	FROM LIFT_LOG ll
	JOIN ATHLETE a ON ll.athlete_id = a.athlete_id
	GROUP BY a.athlete_name -- oh. wait. Is this going to be a problem? Maybe we need to group by ll.athlete_id
	ORDER BY total_weight DESC
	LIMIT 5;

-- It was a problem, but it was a quick fix.
-- I mean, unless we want to combine all Collins into one mega-Collin, then we can flip it back.
...
SELECT a.athlete_name, COUNT(DISTINCT a.gym_home) AS gym_count
	...
	HAVING gym_count > 2; -- I lowered this by one because 4 Collins is crazy... so now there's 3 Collins.
/*
Note: this query catches the duplicate names, but it only keeps a count of distinct gyms. So if two people
with the same name go to the same gym (which is allowed in our design), it wouldn't account for that.
*/
...
select athlete.athlete_id, athlete_name, weight_pounds, gender, height from lift_log
inner join athlete on lift_log.athlete_id = athlete.athlete_id
where lift_log.exercise_id = 1
order by weight_pounds desc;
-- possibly add athlete_id just to differentiate some same-name athletes

#9 FIX ~~~~~~~~~~~~~~~~ State ~ Select the total number of athletes from each state (*****GET DUMMY DATA TO TEST*****)
...
from state inner join
	...
	from location inner join (select athlete.athlete_name, athlete.athlete_id, gym.gym_id, gym.location_id  
	from athlete inner join gym
	on athlete.gym_home = gym.gym_id) as ath2gym
	on location.location_id = ath2gym.location_id) as ath2state
...
*/
select athlete_name, dob, sponsor_name from athlete
inner join sponsor on athlete.sponsor_id = sponsor.sponsor_id;
-- I just pulled out "weight" so that it would run. We removed the "weight" attribute and just left class_id
-- since it broke 3NF. We could do a join WEIGHT_CLASS table and display the class_name to sort of get close.

-- Here's another medium query:
...
-- I was trying to get it to show 0 or null for counties that are in the DB but don't have any gyms, but I
...
-- Note: if we're looking at only gyms in Wisconsin, maybe we don't need to list s.state_name. It's fine either way, though.
...
-- Note: sort of matches with Medium 5 "Show all athlete names that exist in at least 3 different gyms
-- (different athletes that share the same name)"
